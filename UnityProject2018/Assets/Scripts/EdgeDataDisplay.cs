    ...
    private void MaintainLabelText()
    {
        ...
        if (edgeData != null)
        {
            ...
            if (MouseOrbit.Instance.targetInFocus == gameObject)
            {
                ...
                edgeLabelObject.GetComponent<TextMeshPro>().fontSize = 36.0f * distanceToCameraMultiplier;
            }
            else
            {
                edgeLabelObject.GetComponent<TextMeshPro>().alpha = Mathf.Clamp(distanceToCameraMultiplier, 0.2f, 0.7f);
                edgeLabelObject.GetComponent<TextMeshPro>().fontSize = 32.0f * distanceToCameraMultiplier;
            }
